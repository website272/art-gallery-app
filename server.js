...
const JWT_SECRET = 'super_secret_gallery_key_2026'; 
...
// Safely sync database
sequelize.sync()
    .then(() => console.log('Database synced successfully.'))
    .catch(err => console.error('Sync skipped/failed, moving on:', err.message));
...
app.delete('/api/artworks/:id', async (req, res) => {
    try {
        const artwork = await Artwork.findByPk(req.params.id);
        if (!artwork) return res.status(404).json({ error: 'Artwork not found' });
        if (artwork.imagePublicId) await cloudinary.uploader.destroy(artwork.imagePublicId);
